...
for provider in data['providers']:
    ...
    event = events[0]
    if event['status'] not in ('announced', 'confirmed_completed'): raise ValueError('Unsupported event status')
...
completed_today = [record['name'] for record in records if not record['announced'] and timestamp(record['confirmedAt']).date().isoformat() == today]
if today_plans:
